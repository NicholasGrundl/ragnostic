...
def check_file_size(filepath: Path, max_size: int) -> Optional[ValidationCheckFailure]:
...
def check_mime_type(filepath: Path, supported_types: list[str]) -> Tuple[Optional[str], Optional[ValidationCheckFailure]]:
    """Check if file mime type is supported.
    
    Returns:
        Tuple of (mime_type, check_failure). If successful, check_failure will be None.
    """
    try:
        mime_type = magic.from_file(str(filepath), mime=True)
        if mime_type not in supported_types:
            return mime_type, ValidationCheckFailure(
                filepath=filepath,
        ...
        return None, ValidationCheckFailure(
            filepath=filepath,
            check_type=ValidationCheckType.OTHER,
            message=f"Unable to determine mime type: {str(e)}"
        )

def check_duplicate(filepath: Path, file_hash: str, db_client: DatabaseClient) -> Optional[ValidationCheckFailure]:
